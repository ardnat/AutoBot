...
def is_known_username(username):
    ...
    '''
    known_usernames = ['arda', 'sdafnsajdfnioasnfdoin2i3n23n']
...
def generate_response_chat(message_list):
    ...
    return agent.run(message_list[-1])
...
def conversation_tracking(text_message, user_id):
    ...
    # Generate response
    response = generate_response_chat([text_message])
    responseStr = f"output:{response['output']} \n\n\n\n" # ```steps:{response['intermediate_steps']}```"
    # task = generate_response_chat.apply_async(args=[conversation_history])
    # response = task.get()

    # Add the response to the user's responses
    user_responses.append(responseStr)
...
def echo_message(message):
    ...
    responses = conversation_tracking(message.text, user_id)

    # Reply to message
    #split message into 4096 character chunks (telegram max message size)
